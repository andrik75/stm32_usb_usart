...
static void I2CDriver_RxEventCallback(I2CDriver_t* p_i2c_driver, uint16_t dma_curr_pos) {
    ...
    int16_t last_chunk_size = (int16_t)(dma_curr_pos - p_i2c_driver->rx_fifo._head);
    if (last_chunk_size >= 0) {
        if (p_i2c_driver->on_data_received != NULL) {  
            p_i2c_driver->on_data_received(p_i2c_driver, p_i2c_driver->rx_fifo._data + p_i2c_driver->rx_fifo._head, last_chunk_size);
        }
    } else {
